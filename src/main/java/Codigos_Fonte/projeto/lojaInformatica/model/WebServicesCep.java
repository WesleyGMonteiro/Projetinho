...
/**
 * Ferramenta de busca de CEP, veja o m�todo {@link WebServicesCep#searchCep(String)} para
 * maiores informa��es.
 * <BR>
 * <BR>Constroi um objeto {@link WebServicesCep} com os dados XML encapsulados, a partir
 * da chamada do m�todo estatico {@link WebServicesCep#searchCep(String)}.
 * <BR>
 ...
 * <BR>
 * <BR>Exemplo de uso:
 * <BR><tt>{@link WebServicesCep} cep = {@link WebServicesCep}.searchCep("13345-325");
 * 
 ...
 */
public final class WebServicesCep {
...
/* Classes Internas, que auxiliam na busca do CEP */
	...
	private enum Xml {
		CIDADE {
			@Override public void setCep(String text, WebServicesCep webServiceCep) {
		...
		UF {
			@Override public void setCep(String text, WebServicesCep webServiceCep) {
				webServiceCep.setEstado(text);
			}
		}
		;
		/**
		 * Seta o texto enviado no parametro <tt>text</tt> no objeto
		 * {@link WebServicesCep} no seu parametro correspondente. Cada constante do enum
		 * conhece o seu parametro a passar.
		 * @param text {@link String} contendo o texto a ser inserido.
		 * @param webServiceCep {@link WebServicesCep} referencia do objeto para inserir
		 * 		  o parametro text.
		 */
		public abstract void setCep(String text,WebServicesCep webServiceCep);
	}
	/**
...
/* M�todos e variaveis estaticas, respons�veis pela busca do CEP */
	 ...
	 * @return {@link WebServicesCep} contendo as informa��es da pesquisa.
	 */
	public static WebServicesCep searchCep(String cep) {
		cep = cep.replaceAll( "\\D*", "" ); //To numeric digits only
		if (cep.length() > 8)
			cep = cep.substring(0, 8);
		WebServicesCep loadCep = new WebServicesCep(cep);
		try {
...
/* Campos internos de resultado da busca */
	 ...
	 * @param cep
	 */
    private WebServicesCep(String cep) {
    	this.cep = cep;
    }
